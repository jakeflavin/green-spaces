...
import type { Memory, MemoryType, MapBounds, FilterValue } from './types/memory'
import Header from './components/layout/Header'
import MapView from './components/features/MapView'
import type { LatLngBounds } from './components/features/MapView'
import Sidebar from './components/features/Sidebar'
import BottomSheet from './components/ui/BottomSheet'
import Drawer from './components/ui/Drawer'
import MemoryCard from './components/features/MemoryCard'
import AddMemoryPanel from './components/features/AddMemoryPanel'
import IntroModal from './components/features/IntroModal'
import { FilterBar } from './components/ui/FilterBar'
import { MemoryListItem } from './components/ui/MemoryListItem'
...
export default function App() {
  ...
  const filteredMemories = useMemo(
    () => activeFilter === 'all' ? memories : memories.filter((m) => m.type === activeFilter as MemoryType),
    ...
    >
      ...
      <BottomSheet open={isListOpen && !hasActivePanel} onClose={() => setIsListOpen(false)}>
        <FilterBar
          activeFilter={activeFilter}
          onFilterChange={setActiveFilter}
          className="grid grid-cols-3 gap-1.5 px-3 py-2.5 border-b border-gs-border dark:border-gs-border-dark"
        />
